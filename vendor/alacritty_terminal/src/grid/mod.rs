...
impl<T: GridCell + Default + PartialEq> Grid<T> {
        ...
        D: PartialEq,
    {
        ...
        if region.start == 0 {
            ...
            // Rows crossing the active boundary become read-mostly history.
            // Collapse only their repeated suffix; live rows stay in the
...
impl<T> Grid<T> {
    ...
    /// Release rows cached outside the active grid.
    #[inline]
    pub fn compact_history(&mut self) {
        self.raw.compact();
    }

    /// Estimated shallow bytes held by rows cached outside the active grid.
    #[inline]
    ...
    pub fn estimated_storage_bytes(&self) -> usize {
        self.raw.estimated_storage_bytes()
    }

    /// Number of history rows using the compact repeated-suffix form.
    #[inline]
    ...
    /// Physical cell slots allocated across active rows and history.
    #[inline]
    pub fn allocated_cell_capacity(&self) -> usize {
        self.raw.allocated_cell_capacity()
    }

    /// Recompact all retained history, primarily after width reflow thaws it.
    fn compact_retained_rows(&mut self)
    where
        T: Clone + PartialEq,
    {
