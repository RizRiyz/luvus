...
impl<T> Term<T> {
    ...
    /// Release inactive cached rows in both the primary and alternate grids.
    pub fn compact_history(&mut self) {
        self.grid.compact_history();
    ...
    /// Estimated shallow allocation for both terminal grids.
    pub fn estimated_grid_bytes(&self) -> usize {
        self.grid.estimated_storage_bytes().saturating_add(self.inactive_grid.estimated_storage_bytes())
    }

    /// History rows currently stored with a compact repeated suffix.
    pub fn compacted_history_rows(&self) -> usize {
        self.grid.compacted_history_rows() + self.inactive_grid.compacted_history_rows()
    }

    /// Physical cell slots allocated across both terminal grids.
    pub fn allocated_cell_capacity(&self) -> usize {
        self.grid.allocated_cell_capacity() + self.inactive_grid.allocated_cell_capacity()
    }
