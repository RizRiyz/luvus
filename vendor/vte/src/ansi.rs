...
/// Maximum number of bytes buffered in one synchronized update (2MiB).
...
mod tests {
    ...
    #[test]
    fn sync_buffer_is_allocated_lazily() {
        let parser = Processor::<TestSyncHandler>::new();
        assert_eq!(parser.state.sync_state.buffer.capacity(), 0);
    }
