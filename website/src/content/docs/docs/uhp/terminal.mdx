...
Each frame also carries `cursor`, either `null` or a Unicode-scalar `offset` in
the normalized rendered text. ANSI control bytes do not count toward the
offset, so semantic clients can place a caret after applying terminal styles.
