...
selection as the TUI. `pane.split` takes optional `direction`: `"auto"`
(default; split the longer side of the target pane using the last interactive
client size, or side by side when no client has painted), `"right"`, or
`"down"` (`"stack"` is an alias for `"down"`).
