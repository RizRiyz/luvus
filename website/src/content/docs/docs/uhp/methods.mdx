...
`graphics` answers two different questions and both matter:

```json
{"protocol":"kitty","placement":"unicode_placeholder",
 "supported":true,"available":false}
```

`supported` says the build speaks the kitty graphics protocol. `available`
says whether an image would reach a screen *right now*, which depends on the
attached clients: a pane can only display an image while a client whose own
terminal can draw one is attached, and that changes as clients attach and
detach. A program running in a pane does not need to read this — it asks the
terminal directly with the protocol's own query action, and Luvus answers for
the clients attached at that instant.
...
The workspace must belong to the task's recorded project. `branch` is valid
