...
`agent_name` is the operator alias set by `agent.name`; a terminal-backend title
...
Alias changes do not emit an event or advance `event_sequence`: refresh the
snapshot to observe them and do not use its event sequence as an alias ETag.
