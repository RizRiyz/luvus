...
Include `workspace_id` when more than one project is open. A request originating
inside a Luvus pane may instead include its stable `pane` id. `task.next` filters
the ready queue to that project, and path leases collide only between tasks in
the same project. Git worktrees share the repository common directory, so they
still coordinate with each other while unrelated repositories do not.
