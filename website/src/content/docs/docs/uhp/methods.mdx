...
| agent automation | `automation.create` · `automation.list` · `automation.get` · `automation.update` · `automation.enable` · `automation.disable` · `automation.delete` · `automation.run` · `automation.history` · `automation.preview` · `automation.health` |
...
The pane, terminal lifetime, agent ID, and workspace ID must all match. Existing
targets are process-bound, create no ORCH task, and report `delivered` after the
prompt reaches the pane input queue. Server restart or pane closure disables
them rather than retargeting another agent.
