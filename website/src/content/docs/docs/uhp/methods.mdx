...
When `uhp.capabilities` is called through a paired Access gateway, the result
adds `access.mode`, `access.allowed_methods`, and gateway-specific
`access.limits` (`connections`, `requests_per_minute`). The owner method catalog
and contracts are preserved; the owner endpoint omits `access`. Use the
intersection of supported and allowed methods when enabling a client action.
See [effective permissions](/docs/uhp/remote-access/#effective-permissions).
...
```json
→ {"id":"1","method":"pane.split","params":{"direction":"auto"}}
...
| machines (`uhp access --machines` only) | `machine.list` · `machine.get` · `machine.status` · `machine.sessions` · `machine.add` · `machine.rename` · `machine.enable` · `machine.disable` · `machine.remove` |
...
| agents | `agent.list` · `agent.get` · `agent.explain` · `agent.report` · `agent.release` · `agent.start` · `agent.prompt` · `agent.wait` · `agent.name` · `agent.fork` · `agent.send` · `agent.keys` · `agent.read` · `agent.sessions` · `agent.resume` · `pane.report_session` · `pane.release_session` · `pane.report_event` |
...
| terminal | `terminal.backend.inventory` · `terminal.backend.snapshot` · `terminal.backend.validate` · `terminal.backend.processes` · `terminal.backend.capture` · `terminal.backend.observe` · `terminal.backend.control` · `terminal.backend.type_literal` · `terminal.backend.paste_text` · `terminal.backend.submit_text` · `terminal.backend.send_key` · `terminal.backend.set_title` · `terminal.backend.notify` · `terminal.backend.create` · `terminal.backend.close` · `terminal.backend.wait_change` · `terminal.backend.wait_output` · `terminal.backend.events.subscribe` |
| worktrees | `worktree.list` · `worktree.create` · `worktree.open` · `worktree.remove` |
| orchestration | `task.add` · `task.list` · `task.get` · `task.claim` · `task.next` · `task.start` · `task.heartbeat` · `task.update` · `task.done` · `task.retry` · `task.merge` · `task.release` · `lease.acquire` · `lease.list` · `lease.release` |
...
`$LUVUS_PANE_ID` when available. Except for the project-scoped task methods
described below, a raw API request with no pane uses the currently focused
pane. Methods with a focus default also treat an explicit `null` pane as
omitted. A well-formed explicit ID that does not name a live pane fails with
`not_found`; malformed or out-of-range IDs fail with `invalid_request` before
any mutation. Pane IDs accept an unsigned 32-bit JSON integer or its decimal
string form.

Machine methods are an opt-in access-gateway surface, not methods of the
selected session server. `machine.list` takes `{}`. `machine.get`,
`machine.status`, and `machine.sessions` take `{id}`. `machine.add` takes
`{id, host, if_revision, label?, preferred_session?, remote_binary?, enabled?}`.
`preferred_session` selects the remote named session and defaults to `default`.
Rename takes
`{id, label, if_revision}`; enable, disable, and remove take
`{id, if_revision}`. Read projections omit SSH destinations and executable
paths, but include `automatic_provisioning` so a client can distinguish a
managed binary from an explicit path. Control methods are advertised only by
`luvus uhp access --machines --control`.

`machine.add` and `machine.enable` validate remote compatibility but never
install through UHP. A saved `automatic_provisioning:true` is provenance, not
installation authority. If a binary is missing or incompatible, run foreground
CLI setup and approve its default-No prompt, or pass `--install` explicitly for
that operation. Installation never restarts a running remote server.
`machine.status` and all background reconnects remain read-only. Use
`enabled:false` to save without contacting the host.
...
`task.add` binds the task to the selected workspace's project and returns a
`project` object containing its preferred `workspace_id` and durable root.
Include `workspace_id` when project selection is ambiguous. The sole focused
Git project remains implicitly selectable beside a non-Git launch-directory
workspace. A request originating inside a Luvus pane may instead include its
stable `pane` id. `task.next` filters the ready queue to that project, and path
leases collide only between tasks in the same project. Git worktrees share the
repository common directory, so they still coordinate with each other while
...
`task.update` and omit the heartbeat when context-window usage is unknown.
Notes may contain ordinary LF line breaks; other terminal control characters
are rejected. Manual worker launch passes the complete briefing directly to the
agent process instead of typing it through the shell, so stored prompt and note
line breaks remain intact. The CLI exposes the same field as the clearer
`--context-used` flag.
...
`pane.release_session` removes only an exact `{pane, agent, session_id}`
binding. Repeating an already completed release is harmless. A delayed release
for an older session fails with `ownership_conflict` instead of clearing the
newer owner:

```json
{"id":"release","method":"pane.release_session","params":{"pane":"9","agent":"opencode","session_id":"ses_123"}}
```
...
`mode: "workspace"`; omitted mode means worktree. Either mode may include a
stable `workspace_id`; worktree mode creates its branch from that workspace's
repository using the configured `worktree.provider`, while workspace mode
creates a dedicated branchless task tab there. The same provider is used by
`worktree.create`, TUI creation, and explicit `worktree.remove`. A non-`git`
value is the canonical id of an enabled module declaring `[worktree_provider]`;
creation always uses its fixed `command`, while explicit removal uses its
optional `remove_command` and otherwise falls back to Git. Provider selection is
host configuration, not a per-request executable or command parameter. Internal
rollback and task merge remain Git-backed. The workspace must belong to
the task's recorded project. `branch` is valid
only in worktree mode, and `task.merge` returns
`merge_unavailable` for a workspace worker. `task.start` also accepts
`focus: false` to create and start the worker without changing the operator's
current workspace, tab, pane focus, or zoom state.

`task.retry` accepts `{"id":"t1"}` for terminal `done`, `failed`, `review`, or
`blocked` work. Manual tasks return to `queued` with an incremented `attempt`
and bounded `previous_attempts`. Automation-owned tasks return a new
`automation_run` whose `retry_of` points to the preserved original record.
Retrying from `review` or `blocked` finishes the superseded source run as
`cancelled` so it cannot block the retry.

```json
{"id":"1","method":"task.start","params":{"id":"t3","mode":"workspace","workspace_id":"workspace_example","agent":"codex","focus":false}}
```
...
agent adapter. Disabling a definition prevents later occurrences but never
kills a running pane. Detection-only manifest agents cannot be scheduled.
Omitted `task.mode` defaults to the isolated `worktree` mode; shared
`workspace` mode must be explicit. Omitted `task.access` defaults to
`workspace`. Access is independent of worker mode: worktrees isolate Git
history, while `read_only`, `workspace`, and `full_access` select a reviewed
one-shot permission profile owned by the agent adapter. Unsupported combinations
return `unsupported_automation_access` before any ORCH resource is created.

All durable deadlines and occurrence keys are UTC Unix seconds. Calendar
triggers retain an IANA timezone and a local `second_of_day`, so `08:00` stays
08:00 through daylight-saving changes. ISO weekdays use Monday `1` through
Sunday `7`. Intervals must be at least 60 seconds. `automation.preview` returns
the next five UTC deadlines without storing or launching anything.
...
`automation.create` and `automation.run` support retry-safe idempotency keys.
`automation.rebind` is intrinsically idempotent when repeated with the same
verified pane.
Reusing a key with identical input returns the original object; reusing it with
different input returns `idempotency_conflict`. Definitions are capped at 256,
run history at 2,048, and idempotency records at 256. Missed occurrences use
`skip` or `run_latest`; overlap uses `skip` or one bounded `queue_one` run.
Task prompts are bounded to 32 KiB and reject terminal control characters
because Luvus eventually delivers the generated briefing to an interactive
agent pane. Scheduled commands remain visible in that pane but run through the
adapter's non-interactive profile. Luvus does not modify global agent settings
or synthesize approval keystrokes; a detected attention prompt moves the linked
ORCH task to `blocked` and is reflected in automation history.

`target` defaults to `{"kind":"new_worker"}`. An existing live agent uses
`{"kind":"active_agent","pane_id":7,"terminal_id":"<32 lowercase hex>","if_busy":"wait"}`.
The pane, terminal lifetime, agent ID, and workspace ID must all match. Targets
with a trusted native session identity can recover across restart. Public
responses expose only `binding: "durable"` and `target_state` (`bound`,
`restoring`, or `needs_rebind`), never the native session ID or recovery path.
`automation.rebind` accepts only a pane proving the same private conversation.
Targets without that proof remain process-bound. Active targets create no ORCH
task and report `delivered` after the prompt reaches the pane input queue.

`agent.list` returns the stable `workspace_id` and process-lifetime
`terminal_id` on each live agent row so clients can construct this target
without consulting terminal-backend methods.

The read-only methods require the `read` scope. Every mutating automation
method requires `orchestration`. A remote client should discover this contract
through `uhp.capabilities`, subscribe from its returned sequence fence, and
reconcile with `automation.list`, `automation.history`, and
`automation.health` after reconnecting.
...
selection as the TUI. `pane.split` takes optional `direction`: omitted or `"auto"` splits the longer
physical side of the target pane using the last interactive client size. Cell
pixels come from the display client when the host reports them; otherwise Luvus
uses a documented 2:1 cell height/width. With no painted client it splits side
by side. `"right"` and `"down"` force a direction (`"stack"` is an alias for
`"down"`). Older servers reject `"auto"` and treat an omitted direction as
right, so callers that want the server default should omit the field.

`direction` is now validated: any other value, or a non-string, returns
`invalid_request`. Earlier servers silently treated an unrecognized value as
`"right"`, so a caller that sent something like `"vertical"` used to get a
side-by-side split and now gets an error. Send `"right"` explicitly, or omit the
field, to keep that behavior.
...
Every snapshot terminal row includes `pane_id` and nullable `agent_name`.
It also includes nullable `agent_session_title`: the bounded, single-line title
used by the AGENTS sidebar for a live agent pane (OSC title first, then a
module-provided pane or session title). It is `null` when no title is available
or the pane is not an agent. Native `kind:"view"` rows omit it.
OSC and module title changes emit `agent.title_changed`. OSC events carry
`pane` and nullable `title` so clients can update that row directly; module
title events have no per-pane payload and require a snapshot refresh. Neither
requires polling terminal output.
`agent_name` is the operator alias set by `agent.name` or `pane.rename`; a terminal-backend title
does not mask it. This differs from the presentation name in `agent.get` and
`agent.list`, which prefer a backend title. Clearing an alias returns JSON
`null`; native `kind:"view"` rows omit `agent_name`. Workspace and tab indices
remain 1-based in this snapshot (agent-list workspace indices are 0-based).
`agent.name` changes aliases without emitting an event or advancing
`event_sequence`. `pane.rename` updates the same alias map, emits `pane.renamed`,
and advances the sequence, including when clearing a name. Refresh snapshots to
observe aliases; the sequence is not an alias ETag because `agent.name` is silent.
Older servers may omit the field; retain pane-ID display as a fallback.
...
`agent.wait` takes either `{pane, status, timeout_s?}` or
`{pane, statuses, timeout_s?}` and parks on the server. `statuses` is a non-empty
set of `idle`, `working`, `blocked`, and `done`; the response's `status` is the
state that actually matched. The initial comparison and registration are atomic
on the app loop, so it has no subscribe-then-check race and performs no polling.
`luvus wait agent-status` accepts repeated or comma-separated `--status` values.
Against an older server it keeps the subscribe-first event stream for the full
status set. Bounded status polling remains only when the local transport cannot
safely bound a stream read (currently Windows named pipes).
...
`wait:true`. Paste and Enter remain one PTY queue action. Without `wait`, the
response remains immediate: `submitted:true`, `matched:false`, `evidence:"queued"`,
...
same engine lock for both `source:"visible"` and recent output. `source:"visible"`
returns the **live** screen: it reports what the agent is painting now and never
follows the pane's scrollback position, so a read taken while someone is scrolled
back into an earlier turn still describes the frame the fenced keys will land on. A deferred pane
...
`observed_state` identifies the first active transition, `status` is the current
state, and `evidence:"state_transition"` explains a matched result. Transitions
are captured on the app loop even when working and settling happen between
workflow ticks. State evidence is not a native per-prompt turn identifier.

The absolute `timeout_s` deadline (default 300 seconds) covers both observing the
transition and reaching `until` (default `idle`, `done`, or `blocked`). Repeated
`until` entries remain accepted. Timeout returns `submitted:true`, `matched:false`,
`evidence:"timeout"`, and `observed_state` (null when no active transition was seen).
Agents without detectable state transitions therefore time out. The standalone
`agent.wait` retains its current-state matching and status-set semantics.

Pane or terminal exit while a prompt wait is pending returns `agent_not_running`.
Its `error.data` retains `pane`, `queued:true`, `submitted:true`, `observed_state`,
`reason:"pane_closed"`, `baseline_revision`, and `content_revision`. CLI exit codes
remain 0 for a matched wait or no-wait queue admission, 1 for request errors, and
2 for timeouts or pane exit during a wait.

Only one waiting prompt may own a pane. A conflicting waiting request returns
`agent_prompt_busy` before input is queued. No-wait requests retain their existing
behavior and do not acquire wait ownership. Cancellation, timeout, and exit release
that ownership without resending or retracting queued input. Never resend a prompt
automatically after a timeout or lost response because it may already have executed.
Older wait responses omit `observed_state` and can report `output_settled`; callers
requiring the new transition gate should check the response shape.
...
states, locations, structured usage counters, aggregate tokens and cost, and a
read-only `automation` health object with definition, scheduled, live, review,
failed, and next-run counts/deadline.
...
A row needs `text`, or `spans` with at least one non-empty span; everything
else is optional. The first `push` mounts the dock into `placement`
...
`tone` colors the row text with a [Luvus Bar tone](#luvus-bar), resolved
through the active theme. `spans` draws the row from segments instead of
`text`, left to right, each in its own `tone` or else the row's. A row with
neither keeps the default row color, which is not the same as `normal`. An
unknown tone name falls back to the default color rather than failing the
push. A span with no `text` is dropped, so a malformed list falls back to
`text`. `text` is the plain string a click hands to the action as
`LUVUS_MODULE_ROW_TEXT`. A row that sends `spans` and no `text` gets `text`
from the spans joined in order, so a spans-only row still names itself to its
action. A row with neither renders empty and hands its action an empty string.
