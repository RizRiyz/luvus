...
checksum and fleet capability, installs only under `~/.local/bin`, and never
restarts a running remote server. `machine.status` and all background reconnects
remain read-only. Use `enabled:false` to save without provisioning.
