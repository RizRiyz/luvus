...
`text`. `text` stays the plain string a click hands to the action as
`LUVUS_MODULE_ROW_TEXT`; when a row sends `spans` and no `text`, `text` is the
spans joined in order.
