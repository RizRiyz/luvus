...
physical side of the target pane using the last interactive client size and a
documented 2:1 cell height/width (or side by side when no client has painted),
`"right"`, or `"down"` (`"stack"` is an alias for `"down"`). Older servers
reject `"auto"` and treat an omitted direction as right, so callers that want
the server default should omit the field.
