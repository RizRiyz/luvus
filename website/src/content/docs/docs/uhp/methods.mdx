...
are rejected, and a stored multiline note is folded into one paragraph only
when it is included in a later worker launch. The CLI exposes the same field as
the clearer `--context-used` flag.
