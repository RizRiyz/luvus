...
`agent.prompt` takes `target`, `text`, optional `confirm` (default `true`),
`wait` (default `false`), `until`, and `timeout_s`. `until` and `timeout_s`
require `wait:true`. The default until set remains `idle`, `done`, `blocked`.
...
Confirmation reads the detected composer region, or the cursor's logical
soft-wrapped input line, capped at eight text rows. Selection rules join soft
wraps, preserve hard breaks and combining characters, and skip wide spacers.
Trailing row spaces and composer padding are removed, and a leading `›` with
at most one following space is stripped once. Matching is case-sensitive exact
full text, exact first line, or an exact prefix filling the known input-cell
capacity. There is no transcript search. A fresh content revision is required,
even if the same text was visible before paste. Agent state events are not
