...
`worktree.create` and TUI creation. A non-`git` value is the canonical id of an
enabled module declaring `[worktree_provider]`; provider selection is host
configuration, not a per-request executable or command parameter. The
workspace must belong to
