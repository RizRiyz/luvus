...
Case-insensitive global fuzzy matching, exact `luvus search`, and pane-local
content search share Unicode default simple case folding. For example, Greek
sigma variants match consistently, while dotless `ı` stays distinct from `i`.
Search does not apply locale-specific Turkic folding, multi-character expansion
such as `ß` to `ss`, or Unicode normalization. `Ctrl+i` still changes case
sensitivity only for retained **Output** matching; navigation and file paths
...
press `Alt+A` or `F2`. The shared FILES menu offers native views and available
editors. The menu appears over the finder;
...
The retained-output command remains exact rather than fuzzy. Its query is
limited to 4 KiB in UTF-8 bytes; the CLI rejects longer queries before sending
them to the server:
