...
toast. In a mouse-aware TUI, Luvus distinguishes a click from a drag: a click is
replayed to the application on release, while a drag remains Luvus selection.
This prevents an application that enables mouse reporting only for buttons and
hover from swallowing drag-to-copy. Turn off **Mouse drag selects text** in
**Settings → General** when an application needs to own plain drag gestures. In
that compatibility mode, **Shift+drag** still selects through Luvus.

Luvus requests both your native OS clipboard (`pbcopy` / `wl-copy` /
