...
reach the child process. Type a query and press **Enter** to jump to the first
case-insensitive match at or after the current viewport (wrapping to the oldest
match when needed). Press **`Ctrl-I`** to toggle case sensitivity; sensitive
...
word-level highlights as SCROLL search. Press **Enter** to commit and move the
copy cursor to the first match at or after its current position; the selection
