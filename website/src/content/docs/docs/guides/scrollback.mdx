...
| `/` | search retained output in this exact PTY pane |
| `q`, `Esc`, `0`, or just typing | back to live |

The inline `/` search is owned by Luvus, so query and navigation keys never
reach the child process. Type a query and press **Enter**; retained rows are
scanned synchronously, and the first case-insensitive match at or after the
current viewport is selected (wrapping to the oldest match when needed). Press
**`Ctrl-I`** to toggle case sensitivity; sensitive
mode adds `Aa` after the query, while the default insensitive mode stays
unmarked. Press **`Ctrl-U`** while editing or after committing to clear the
query and continue editing without leaving search or moving the viewport.
Matches are highlighted in place like FILE search: the current word uses the
accent color, and other visible hits use amber. Press
**`n`** / **`N`** for the next / previous match; both wrap within that pane's
retained rows. **Esc** while editing or after committing
cancels the search and restores the viewport from before `/`, leaving keyboard
scroll mode active; press **Esc** again to return to live output. A no-match
query remains active as `0/0` until canceled. Search retains at most 10,000
matches; a `+` after the count means additional retained-output matches exist
beyond the navigable set. Terminal output or a pane reflow invalidates committed
coordinates and returns the same query to editing rather than navigating stale
rows.

This pane-local search is separate from the prefix **global fuzzy finder**. The
configured prefix followed by `/` still searches sessions, workspaces, tabs,
panes, agents, files, and retained output across the selected Luvus home.
...
| `Ctrl+D` / `Ctrl+U` | half a page down / up when search is closed |
| `/` | search retained output from the copy cursor |
...
Copy mode's `/` search uses the same exact, case-insensitive matching and
word-level highlights as SCROLL search. Press **Enter** to commit; the copy
cursor moves synchronously to the first match at or after its current position.
The selection
anchor stays unchanged. Use **`n`** / **`N`** to move between matches and
**`Ctrl-I`** to toggle case sensitivity. While SEARCH is active, **`Ctrl-U`**
clears the query and returns to editing instead of moving half a page. **Esc**
clears SEARCH and returns to COPY; press **Esc** again to cancel COPY and restore
its entry viewport. After committing a search, **`y`** or **Enter** copies the
selection and returns to live output.
