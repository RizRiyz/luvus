...
| Tab | What's there |
|---|---|
| **General** | File opening, hidden files, modified Enter, update and resume behavior, **Mouse drag selects text**, then notification styles and tests. The mouse option keeps clicks working in mouse-aware TUIs while Luvus owns plain drag-to-copy. |
...
and then `?` to show the current map. The Keyboard Shortcuts panel clearly
separates **Prefix commands** from opt-in **Direct shortcuts**. Direct shortcuts
are read from `direct_keybindings`; their labels are normalized for display,
and the panel lists only the effective winner when multiple commands configure
the same chord. See [Keybindings](/docs/reference/keybindings/) for the full
list and direct-shortcut configuration format.
