...
the placeholder is replaced by the normal live agent row when its ORCH task
starts. Clicking a scheduled placeholder or a Mission Control automation row
...
from the same built-in launch-capable registry used by ORCH. Task and Automation
...
| Scheduled worker stops at folder trust or approval | Trust the workspace in that agent or configure its normal automation-safe permission mode before arming the schedule. Permission bypass remains an explicit agent-level choice. |
