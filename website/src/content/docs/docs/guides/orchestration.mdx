...
definition's missed-run policy.
...
The header switches between **Tasks** and **Automations**. The automation view
shows the definition state, retained schedule, next UTC deadline, target agent,
and target workspace. Use `e` to pause or re-enable the selected definition,
`r` to request one run without advancing its schedule, and `D` to delete an
idle definition. Armed definitions also appear in the sidebar's Active agents
section as `scheduled`; the placeholder is replaced by the normal live agent
row when its ORCH task starts. Mission Control shows aggregate armed, live,
...
`Run in` choice for an isolated Worktree or the existing Workspace, `Once later`,
...
and Left/Right to change the selected Agent, Run in, or Start value. Agent choices come
from the same built-in launch-capable registry used by ORCH.
...
is due. The form detects the server user's IANA timezone internally without
adding it to the compact form. Schedule input uses that local wall-clock time:
...
- **Only armed agent automations spawn unattended.** Manual tasks still start
  ...
  explicit agent, workspace, prompt, worker mode, and bounded schedule.
- **No arbitrary scheduled shell action.** The scheduler launches only a
  reviewed built-in agent adapter through ORCH; detection-only manifests do not
