...
Luvus pane infer its workspace. With multiple projects, callers outside a pane
must pass `--workspace-id`; use `luvus workspace list` to discover the stable
id. Legacy tasks without project metadata require an explicit workspace when
the session contains more than one project.
...
luvus task next --start --agent "claude" --workspace-id workspace_example
