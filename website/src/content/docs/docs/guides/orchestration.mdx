...
`review` (gate failed, so fix and retry) → `merging` → `merged`, or `blocked`
on a merge conflict. Only one task integrates at a time. An interrupted
integration safely returns to `done` after restart. The ledger lives in
`~/.luvus/orch.json`, separate from your session.
...
6. Select a done task, `m`, and the branch merges into `luvus/integration`
   in an isolated worktree. The task moves through **merging** to **merged** and
