...
hides concrete dependencies and owns a registry-backed Agent, `Once later`,
...
and Left/Right to change the selected Start or Agent value. Agent choices come
...
is due. The form detects and displays the server user's IANA timezone. Schedule
input is local wall-clock time: `YYYY-MM-DD HH:MM` for Once, a minute from
