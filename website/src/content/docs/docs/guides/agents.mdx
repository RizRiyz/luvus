...
Kimi, Grok, Hermes CLI, Letta Code, Muse Code, Arc Studio CLI, Pi, Oh My Pi (OMP),
...
Aider, OpenCode V2, Kilo Code, Kiro, Cursor, Amp, Droid, Muse, Devin, Letta Code, Arc Studio CLI, and
...
luvus reopens each agent's **own conversation** after a restart. For agents
with a stable native store, it discovers the session by pane folder with no
setup. Integrations provide exact ownership where directory matching alone is
not sufficient, and Luvus runs the agent's own resume command:
...
| Devin | its exact session report from the optional integration, resumed with `devin --resume <id>` |
...
an exact binding that was reported and persisted, which restores as
`devin --resume <id>`. The optional integration reports it automatically;
without it, use `luvus pane report --agent devin --session <id>`.
...
```sh
luvus integration install claude    # or: copilot · codex · antigravity · letta · opencode · kimi · grok · hermes · omp · devin
```
...
- **letta**: adds one quiet `SessionStart` entry to
  `~/.letta/settings.json` and one small platform-specific hook script. It
  reports only the exact conversation ID for restart resume. Detection and
  state remain native and work without the integration.
...
- **hermes**: installs an opt-in plugin under the active `HERMES_HOME`. It
  reports an exact session id once per session, which Luvus persists for
  restart resume. Luvus preserves unrelated YAML settings and removes only its
  own plugin entry on uninstall. Without the integration, detection still
  works but historical Hermes sessions are not scanned or guessed.
...
- **devin**: adds one `SessionStart` entry to Devin's user config
  (`~/.config/devin/config.json`, or
  `%USERPROFILE%\AppData\Roaming\devin\config.json` on Windows) and one small
  platform-specific hook script beside it. It reports only the exact session
  ID for restart resume, including after `/clear` or an in-TUI `/resume`.
  Devin rejects a whole hook file for one unknown event, so Luvus writes only
  `SessionStart` and keeps other tools' hooks intact. A config with comments
  is refused instead of rewritten, and files selected with `devin --config`
  are not managed.
