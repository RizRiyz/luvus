...
```sh
luvus integration install claude    # or: copilot · codex · antigravity · opencode · kimi · grok · hermes · omp · pi
```
...
- **pi**: installs one managed TypeScript extension in Pi's active agent
  directory (`~/.pi/agent/extensions/`, honoring `PI_CODING_AGENT_DIR`,
  `PI_CONFIG_DIR`, and `PI_PROFILE`). Same reporting contract as the OMP
  extension: exact session identity plus authoritative
  idle/working/blocked/done transitions, with native detection as fallback.
