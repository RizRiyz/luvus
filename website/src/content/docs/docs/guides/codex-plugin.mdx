...
pane, an explicit `--session` wins over any inherited socket. When that selector
targets another server, Luvus also discards the caller's inherited pane id;
pane ids are valid only in the session that issued them. It does not run
