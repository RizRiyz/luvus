...
lets a provider carry normal UHP 1.0 frames without building Tailcat, WebAssembly,
SSH, or any other transport into Luvus core.
...
explicitly and grants the same limited 15-minute control surface used by the
...
WASM runtime, timer, or network activity.
...
The Experimental `luvus web` flow is one sample provider and client:

- native Tailcat forwards the private access endpoint;
- a lazy-loaded Tailcat WASM adapter implements a minimal `dial(port)` byte-stream
  interface;
- the browser's UHP client depends only on that interface, not on Tailcat.

Tailcat and the browser artifact are optional samples under `web/`; they are
excluded from the Cargo package and normal startup. Another provider or client
can replace them without changing UHP or Luvus core.
