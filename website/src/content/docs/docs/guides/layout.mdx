...
sessions found on disk (click one to reopen it). When multiple workspaces are
open, the **Here** chip on the next row scopes both live agents and resumable
sessions to the active workspace. Both choices are remembered across restarts;
use prefix `a` for All/Active and prefix `A` for All workspaces/Here.

Attention stays global while **Here** is selected. If another workspace has a
blocked agent, one overflow line reports the count; click it to jump to that
agent without clearing the scope. The scope chip is hidden when only one
workspace is open. See [Working with Agents](/docs/guides/agents/).
:::
