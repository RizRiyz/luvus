...
- **Reorder:** right-click a row and choose **Reorder**, then drag that row above
  or below another workspace. Its circle becomes a reorder marker while the
  mode is active. Linked worktrees move with their parent as one group, and
  pinned workspaces reorder only among the other pinned groups. The order is
