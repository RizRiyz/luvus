...
With a mouse, right-click a running session that is not current and select
**Stop** to end its server and live panes. Right-click that stopped named session
again and select **Delete** to remove its saved state after confirmation. The
current session cannot be stopped or deleted, and the default session cannot be
deleted. Reopening the sheet shows its last known rows immediately while one
background refresh checks for external changes.
