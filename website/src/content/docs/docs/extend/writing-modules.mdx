...
| `worktree_provider` |  | A single `[worktree_provider]` table declares a fixed synchronous worktree creation command. |
...
### `[worktree_provider]`: create and remove worktrees for Luvus

A module may expose one synchronous creation command and an optional explicit
removal command:
...
The user selects the module by canonical id in
`config.json` as `worktree.provider`; configuration cannot replace this fixed
argv. Luvus runs the command in the module root with normal module identity and
settings variables plus:

| Variable | Meaning |
|---|---|
| `LUVUS_WORKTREE_PROVIDER_VERSION` | Provider contract version, currently `1`. |
| `LUVUS_WORKTREE_OPERATION` | `create` or `remove`. |
| `LUVUS_WORKTREE_REPOSITORY` | Source repository path. |
| `LUVUS_WORKTREE_PATH` | Removal target; unset for creation. |
| `LUVUS_WORKTREE_BRANCH` | Exact requested/target branch. |
| `LUVUS_WORKTREE_BRANCH_EXISTS` | Creation only: `true` when the local branch already exists. |
| `LUVUS_WORKTREE_FORCE` | Removal only: `true` for a confirmed TUI force-delete. |
| `LUVUS_WORKTREE_REQUEST_JSON` | Complete versioned request as JSON. |

Creation returns exactly one JSON object on stdout:

```json
{"path":"/absolute/path/to/worktree"}
```

Removal must write nothing to stdout. It succeeds only after the target has
been unregistered from Git and its directory no longer exists. Luvus validates
that the removal target is a linked worktree of the source repository before
running the command and verifies the result afterwards. If `remove_command` is
omitted, explicit removal uses built-in Git.

Write human logs to stderr. A non-zero exit preserves stderr for the user.
The provider command is synchronous: do not call the Luvus CLI or API from it,
because the server is waiting for the provider result. It may use its own module
settings, config, and state directories normally. Luvus opens no workspace until it verifies that the returned path exists, is a
registered worktree of the source repository, shares its Git common directory,
and checks out the requested branch. Internal rollback and ORCH merge stay
Git-backed. Both fixed provider commands are shown in the module install
confirmation alongside all other executable commands.
...
| Agents | `agent.hook` |
| Orchestration | `task.added` · `task.claimed` · `task.started` · `task.updated` · `task.ready` · `task.done` · `task.released` · `task.retried` · `task.deleted` · `task.merge_started` · `task.merged` · `task.merge_conflict` · `task.merge_failed` · `task.needs_compaction` · `task.gate_running` · `task.gate_passed` · `task.gate_failed` |
...
|---|:---:|---|
| `text` | ✓ | The row label. May be omitted only when `spans` has at least one non-empty segment, then it defaults to the segments joined. A row with neither renders empty and its action receives an empty `LUVUS_MODULE_ROW_TEXT`. |
| `dot` |  | A status dot: `idle` / `working` / `blocked` / `done` (colored like the Agents list). |
| `tone` |  | Colors the row text with a Luvus Bar tone: `normal` / `muted` / `accent` / `success` / `warning` / `error`. Omit for the default row color (`normal` is the bar's own text color, one step dimmer than the default). An unknown name falls back to the default. |
| `spans` |  | Draw the row from segments instead of `text`: `[{"text": "[", "tone": "muted"}, {"text": "━━━", "tone": "success"}]`. Each segment takes its own tone, else the row's. `text` stays the plain version handed to the action, and defaults to the segments joined when omitted. |
...
| `LUVUS_WORKTREE_*` | The versioned request for `[worktree_provider]` commands. |
