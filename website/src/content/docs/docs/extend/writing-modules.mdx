...
### `[worktree_provider]`: create worktrees for Luvus

A module may expose one synchronous worktree creation command:
...
| `LUVUS_WORKTREE_BRANCH_EXISTS` | `true` when the local branch already exists. |
| `LUVUS_WORKTREE_REQUEST_JSON` | Version, repository, branch, and `branch_exists` as JSON. |
...
and checks out the requested branch. Removal and ORCH merge stay Git-backed.
The provider command is shown in the module install confirmation alongside all
