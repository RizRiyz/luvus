...
| `spans` |  | Draw the row from segments instead of `text`: `[{"text": "[", "tone": "muted"}, {"text": "━━━", "tone": "success"}]`. Each segment takes its own tone, else the row's. `text` stays the plain version handed to the action. |
