...
- **Hook**, via `luvus integration install <agent>`, makes the agent report its
  ...
  OMP's XDG session store remain supported. Pi works the same way:
  `luvus integration install pi` drops one managed `luvus.ts` into Pi's active
  agent directory (`~/.pi/agent/extensions/`, honoring `PI_CODING_AGENT_DIR`,
  `PI_CONFIG_DIR`, and `PI_PROFILE`). It reports exact session identity and
  authoritative idle/working/blocked/done transitions while preserving Luvus's
  native process/screen detection as the zero-setup fallback.
