---
title: UHP Terminal Profile
description: Control every live Luvus PTY through the terminal profile of the Universal Harness Protocol.
---

This is the detailed Terminal profile reference for the
[Universal Harness Protocol](/docs/reference/uhp/). Its compatibility wire name
remains `luvus-terminal-backend` 1.0 and its methods remain
`terminal.backend.*`.

The profile lets orchestrators discover every real PTY across all workspaces,
...
Terminal profile's exact contract.
...
running row, validate its `endpoint`, then connect directly and send:

```json
{"id":"cap-1","method":"terminal.backend.capabilities","params":{"protocol":{"name":"luvus-terminal-backend","major":1,"minor":0}}}
```

The response selects the protocol version, returns a random
`server_generation`, and announces the exact capabilities and active limits.
Never call an unannounced capability.
...
Actions reuse the ordinary strict UHP Terminal validation and replies. Only one
...
switch their connections to streams after one acknowledgment. Every complete frame is capped at 1 MiB. Request IDs are
strings of 1 to 128 UTF-8 bytes. Unknown fields, duplicate keys, invalid enums,
...
source. The Runtime profile is independently versioned under
...
failure-injection consumers under `examples/terminal-backend/`. Live tools
...
the versioned
[conformance guide](https://github.com/RizRiyz/luvus/tree/main/protocol/terminal-backend/v1/conformance)
