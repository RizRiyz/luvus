...
For `machine add` and `machine enable`, remote validation does not authorize
installation. Provisioning requires `--install` or an explicit Yes at the
interactive permission prompt; its default is No. Non-interactive callers
must supply `--install` when installation is needed. Existing explicit binary
paths are never replaced. See [Multiple Machines](/docs/guides/machines/).
...
inserted beside that target in the same workspace and tab. With no direction
flag, Luvus omits `direction` on the wire so an older server still accepts the
request and splits side by side; a current server splits along the longer physical side of the target pane,
using the display client's cell pixels when the host reports them, otherwise
treating cells as about twice as tall as they are wide. `--auto`, `--right`, and `--down` send an explicit
direction. It starts in the target pane's live working directory; with
...
`luvus uhp capabilities` on the owner endpoint reports supported methods. The same RPC through paired `luvus uhp access` adds `access.mode`, `access.allowed_methods`, and gateway connection/request limits; use that effective set for permission-aware clients.

`luvus web` is an explicit foreground client bridge, not part of ordinary TUI
or server startup. It starts or reuses the selected session without restarting
it, binds loopback, and serves the embedded browser application with read-only
authority by default. Add `--control` for bounded terminal and workspace input.
Stopping the command revokes only its temporary web authority; the Luvus
server, PTYs, and other clients remain attached.

`luvus uhp snapshot` includes nullable `agent_name` operator aliases on terminal rows alongside `pane_id`. Native view rows omit the field. Refresh snapshots after naming: `agent.name` emits no event and does not advance `event_sequence`, while `pane.rename` updates the same map, emits `pane.renamed`, and advances the sequence, including when clearing a name. The event sequence is not an alias ETag because `agent.name` is silent.
