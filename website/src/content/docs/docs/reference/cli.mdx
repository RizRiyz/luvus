...
`luvus uhp snapshot` includes nullable `agent_name` operator aliases on terminal rows alongside `pane_id`. Native view rows omit the field. Renames require a fresh snapshot; event sequence is not an alias ETag.
