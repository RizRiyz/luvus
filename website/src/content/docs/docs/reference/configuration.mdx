...
| `resume_agent_sessions` | automatically resume captured native agent sessions when restoring the server; defaults to `true`. Turn it off in **Settings → General** to restore those panes as ordinary shells. |
| `resume_launch_flags` | replay each agent pane's captured CLI options during automatic resume; defaults to `false` and has no effect when `resume_agent_sessions` is off |
