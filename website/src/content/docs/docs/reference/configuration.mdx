...
| `worktrees/` | worktrees created by the default Git provider, nested per repo; Worktrunk uses its own configured path template |
...
| `worktree.provider` | worktree creation backend: `git` (default, backward-compatible) or `worktrunk`. This applies to CLI/API creation, TUI creation, and ORCH task worktrees; remove and task merge remain Git-backed. |
| `worktree.executable` | Worktrunk executable name or path; defaults to `wt`. It is executed directly, never as a shell command. |
| `worktree.args` | structured extra argv for Worktrunk. Luvus rejects positional values and arguments that can override repository/config/output, execute another program, change cd behavior, select/create/base a branch, clobber paths, or bypass hook approval. Luvus supplies `-C <repo> switch [--create] <branch> --no-cd --format json`. |
