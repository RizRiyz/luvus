...
| `mouse_drag_select` | distinguish clicks from drags in mouse-aware panes; defaults to `true`, so clicks reach the application while drags select and copy through Luvus. Set to `false` to give the application plain drag gestures; Shift+drag remains Luvus selection. |
