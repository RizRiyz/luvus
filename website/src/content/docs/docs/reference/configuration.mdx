...
| `worktree.provider` | worktree creation backend: `git` (default, backward-compatible) or the canonical id of an enabled module declaring `[worktree_provider]`. Configuration cannot supply executable or argv. This applies to CLI/API creation, TUI creation, and ORCH task worktrees; remove and task merge remain Git-backed. |
