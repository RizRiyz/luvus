...
Start with [Automating with the Socket API and UHP](/docs/guides/socket-and-uhp/)
...
`luvus socket schema` prints the complete installed Socket API 1.0
contract without starting or connecting to a server. `luvus socket
capabilities` queries the selected running server and returns its exact
method registry, event sequence fence, limits, supported UHP profiles, and
graphics support. Discover capabilities instead of inferring support from the
Luvus release number.

The response includes `uhp.enabled`. When it is `false`, `profiles` contains
only `luvus-socket` and the advertised method list omits UHP negotiation and
Terminal-profile entry points. Use `luvus uhp enable` or `luvus uhp disable`
to change this live. The Socket API itself remains available because Luvus's
own CLI uses it.

| Family | Methods |
|---|---|
| discovery / config | `socket.capabilities` · `socket.stats` · `socket.token.create` · `socket.token.list` · `socket.token.revoke` · `config.get` · `config.patch` · `server.reload_config` · `server.agent_manifests` · `server.reload_agent_manifests` |
...
| UHP Runtime profile | `runtime.capabilities` · `session.snapshot` · `agent.start` · `agent.prompt` · `agent.wait` · `events.subscribe` |
...
| UHP Terminal profile | `terminal.backend.capabilities` · `terminal.backend.inventory` · `terminal.backend.snapshot` · `terminal.backend.validate` · `terminal.backend.processes` · `terminal.backend.capture` · `terminal.backend.observe` · `terminal.backend.control` · `terminal.backend.type_literal` · `terminal.backend.submit_text` · `terminal.backend.send_key` · `terminal.backend.set_title` · `terminal.backend.notify` · `terminal.backend.create` · `terminal.backend.close` · `terminal.backend.wait_change` · `terminal.backend.wait_output` · `terminal.backend.events.subscribe` |
...
The Runtime and `terminal.backend.*` families form the
[Universal Harness Protocol](/docs/reference/uhp/). The Terminal profile is a
versioned automation contract and does not
...
[UHP Terminal Profile](/docs/reference/terminal-backend/) reference.
...
```json
→ {"id":"token-1","method":"socket.token.create","params":{"scopes":["read","workspace"],"ttl_s":3600}}
```
...
`socket.stats` exposes bounded operational counters: active/capacity/accepted/
...
The installed Socket schema embeds the strict UHP Runtime and Terminal profile
bundles under `profiles`. Live terminal observe/control streams are capped at
...
[UHP Terminal Profile](/docs/reference/terminal-backend/) for framing and
...
name, and waits for manifest-driven detection without composing separate socket
requests.
...
capacity. See the [UHP Terminal profile](/docs/reference/terminal-backend/) for
