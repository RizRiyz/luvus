...
`luvus api socket-schema` prints the complete installed Socket API 1.0
contract without starting or connecting to a server. `luvus api
socket-capabilities` queries the selected running server and returns its exact
