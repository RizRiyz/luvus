...
In the FILES tree, `o` or `Ctrl+O` opens the selected file or folder in the
desktop's own application, on the machine running the client. The path is
passed as a separate argument, which prevents shell injection but does not
change what the OS handler does: application or shortcut-like entries
(`.app`, `.desktop`, `.lnk`, `.command`) run exactly as a double-click would,
so only open entries you trust. After `Enter` opens a file view, `x` closes
that view. An opened DIFF review closes with `q` or `Esc`.
...
read-only or in an editor, open it externally, explicitly preview Markdown/Mermaid, or manage the
