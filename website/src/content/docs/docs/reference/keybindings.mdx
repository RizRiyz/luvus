...
| `e` | focus FILES tree | `M` | jump palette (tabs / workspaces / agents) |
...
the FILES tree, where arrows or `hjkl` browse it and `Esc` returns to the pane.
After `Enter` opens a file view, `x` closes that view.
