...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="terminal-backend-live-", dir=target) as state:
        ...
        try:
            ...
            assert proxied["result"]["type"] == "runtime_capabilities"
            incompatible = request(socket_path, {"id":"incompatible","method":"terminal.backend.capabilities","params":{"protocol":{"name":"luvus-terminal-backend","major":1,"minor":1}}})
